...
def process_video(cap, model, actions, seq_length, width, height, device, corners):
    ...
    out = cv2.VideoWriter('scenario input5.mp4', fourcc, cap.get(cv2.CAP_PROP_FPS), (width, height))
    out2 = cv2.VideoWriter('scenario output5.mp4', fourcc, cap.get(cv2.CAP_PROP_FPS), (width, height))
